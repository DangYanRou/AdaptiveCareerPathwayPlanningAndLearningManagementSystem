...
@Service
public class CompetencyAssignmentCollaborationServiceImpl implements CompetencyAssignmentCollaborationService {
    ...
    @Transactional
    public List<CompetencyAssignmentCreationRequiredDataDto> getCreationRequiredCompetency(
    ...
    @Transactional
    public void proposeCompetencyAssignment(ProposeCompetencyAssignmentRequest req, UUID userUUID) throws Exception {
        ...
        Map<UUID, StaffDto> reviewerMap = allStaffDtos.stream()
                ...
                .collect(Collectors.toMap(StaffDto::getId, Function.identity()));
    ...
    @Transactional
    public List<RoleCompetencyProposalOverviewDto> getOverview(UUID userUUID) throws Exception {
        ...
        List<RoleCompetencyProposalOverviewDto> reviewingRecord = reviewingRoleCompetencyProposal.stream()
                .map(dto -> {
                    List<JobScopeDto> assignedJobScopes = roleJobScopeProposalParticipated.stream()
                            ...
                            .map(RoleJobScopeProposalDto::getJobScope).toList();
                    ...
                    List<ProposeCompetencyAssignmentMap> assignedPermanentCompetencies = roleCompetencyAssignmentParticipated
                            ...
                            .map(competency -> new ProposeCompetencyAssignmentMap(
                                    ...
                                    competency.getCompetency(), null, competency.getWeightage()
                            )).toList();
                    ...
                    List<ProposeCompetencyAssignmentMap> assignedProposedCompetencies = roleCompetencyAssignmentProposalParticipated
                            ...
                            .map(competency -> new ProposeCompetencyAssignmentMap(
                                    ...
                                    null, competency.getCompetencyProposal(), competency.getWeightage()
                            )).toList();
                    ...
                    int totalWeightage = assignmentCompetencyMap.stream().mapToInt(ProposeCompetencyAssignmentMap::getWeightage).sum();

                    return new RoleCompetencyProposalOverviewDto(
                            dto.getId(), dto.getRole().getOrgChart().getId(), dto.getRole().getOrgChart().getName(),
                            dto.getRole().getId(), dto.getRole().getName(), dto.getDescription(),
                            assignedJobScopes, assignmentCompetencyMap, dto.getStaff().getName(),
                            dto.getStaff().getEmail(), true, participantMap.get(dto.getUpdatedBy()), totalWeightage
                    );
                }).toList();
        ...
        List<RoleCompetencyProposalOverviewDto> proposingRecord = proposingRoleCompetencyProposal.stream()
                .map(dto -> {
                    ...
                    return new RoleCompetencyProposalOverviewDto(
                            ...
                            dto.getRole().getId(), dto.getRole().getName(), dto.getDescription(),
                            assignedJobScopes, assignmentCompetencyMap, dto.getStaff().getName(),
                            dto.getStaff().getEmail(), false, participantMap.get(dto.getUpdatedBy()), totalWeightage
                    );
                }).toList();
    ...
    @Transactional
    public void updateCompetencyProposal(EditCompetencyAssignmentProposalRequestDto req, UUID userUUID) throws Exception {
        ...
        List<JobScopeDto> selectedJobScopes = req.getJobScopeList().stream().map(jobScope -> new JobScopeDto(
                jobScope, false, userUUID, now, userUUID, now
        )).toList();
        ...
        Map<Long, Integer> selectedPermanentCompetencyMap = req.getCompetencyList().stream()
                ...
                .collect(Collectors.toMap(dto -> dto.getId().getCompetencyId(), ProposeCompetencyAssignmentMap::getWeightage));

        Map<ProposalParticipantId, Integer> selectedProposedCompetencyMap = req.getCompetencyList().stream()
                .filter(dto -> dto.getId().isProposal())
                .collect(Collectors.toMap(dto -> dto.getId().getCompetencyProposalId(), ProposeCompetencyAssignmentMap::getWeightage));

        Map<UUID, ProposalParticipantDto> allParticipant = proposalParticipantService.getAllByProposalId(req.getId().getProposalId())
                .stream().collect(Collectors.toMap(dto -> dto.getId().getStaffId(), Function.identity()));
        ...
        if (!selectedJobScopes.isEmpty()) {
            requiredJobScopeMap = jobScopeService.createAll(selectedJobScopes).stream()
                    .collect(Collectors.toMap(JobScopeDto::getId, Function.identity()));
        } else {
        ...
        Set<Long> assignedJobScopeIDs = roleJobScopeProposalService.getByRoleCompetencyProposalId(
                req.getId().getProposalId(), req.getId().getRoleId(), req.getId().getStaffId()
            ...
            List<RoleJobScopeProposalDto> roleJobScopeToBeAdded = jobScopeToBeAdded.stream().map(id ->
                    new RoleJobScopeProposalDto(
                            new RoleJobScopeProposalId(updatedRecord.getId().getProposalId(),
                                    updatedRecord.getId().getRoleId(), updatedRecord.getId().getStaffId(), id),
                            updatedRecord.getProposal(), updatedRecord.getRole(), updatedRecord.getStaff(),
                            requiredJobScopeMap.get(id), userUUID, now, userUUID, now
                    )
            ).toList();
        ...
        if (!jobScopeToBeRemoved.isEmpty()) {
        ...
        Map<RoleCompetencyItemId, RoleCompetencyItemDto> assignedRoleCompetencyMap = roleCompetencyItemService
                ...
                .collect(Collectors.toMap(RoleCompetencyItemDto::getId, Function.identity()));

        Map<Long, CompetencyDto> assignedPermanentCompetencyMap = assignedRoleCompetencyMap.entrySet().stream()
                .collect(Collectors.toMap(map -> map.getKey().getCompetencyId(), map -> map.getValue().getCompetency()));
        ...
        if (!permanentCompetencyIdToAdd.isEmpty()) {
            List<RoleCompetencyItemDto> roleCompetencyItemDtoToAdd = permanentCompetencyIdToAdd.stream().map(id ->
                    new RoleCompetencyItemDto(
                            new RoleCompetencyItemId(updatedRecord.getId().getProposalId(),
                                    updatedRecord.getId().getRoleId(), updatedRecord.getId().getStaffId(), id),
                            updatedRecord.getProposal(), updatedRecord.getRole(), updatedRecord.getStaff(),
                            existingPermanentCompetencyMap.get(id), selectedPermanentCompetencyMap.get(id),
                            userUUID, now, userUUID, now
                    )
            ).toList();
        ...
        if (!permanentCompetencyIdToRemove.isEmpty()) {
        ...
        if (!permanentCompetencyIdToCheck.isEmpty()) {
            Set<RoleCompetencyItemId> roleCompetencyToCheck = permanentCompetencyIdToCheck.stream().map(id ->
                    new RoleCompetencyItemId(updatedRecord.getId().getProposalId(),
                            updatedRecord.getId().getRoleId(), updatedRecord.getId().getStaffId(), id)
            ).collect(Collectors.toSet());
            Map<RoleCompetencyItemId, RoleCompetencyItemDto> roleCompetencyItemToUpdate = roleCompetencyToCheck.stream().filter(
                    id -> !Objects.equals(
                            ...
                            selectedPermanentCompetencyMap.get(id.getCompetencyId()))
        ...
        Map<ProposalParticipantId, CompetencyProposalDto> existingProposedCompetencyMap = competencyProposalService.getAll().stream()
                .collect(Collectors.toMap(CompetencyProposalDto::getId, Function.identity()));
        Map<RoleCompetencyProposalItemId, RoleCompetencyProposalItemDto> assignedRoleCompetencyProposalMap =
                roleCompetencyProposalItemService.findAllByProposalStaffId(req.getId().getProposalId(), req.getId().getStaffId())
                        .stream().collect(Collectors.toMap(RoleCompetencyProposalItemDto::getId, Function.identity()));
        Map<ProposalParticipantId, CompetencyProposalDto> assignedProposedCompetencyMap = assignedRoleCompetencyProposalMap.entrySet().stream()
                .collect(Collectors.toMap(map -> map.getKey().getCompetencyProposalId(), map -> map.getValue().getCompetencyProposal()));
        ...
        if (!proposedCompetencyIdToAdd.isEmpty()) {
            List<RoleCompetencyProposalItemDto> roleCompetencyProposalItemDtoToAdd = proposedCompetencyIdToAdd.stream().map(id ->
                    new RoleCompetencyProposalItemDto(
                            new RoleCompetencyProposalItemId(updatedRecord.getId().getProposalId(),
                                    updatedRecord.getId().getRoleId(), updatedRecord.getId().getStaffId(), id),
                            updatedRecord.getProposal(), updatedRecord.getRole(), updatedRecord.getStaff(),
                            existingProposedCompetencyMap.get(id), selectedProposedCompetencyMap.get(id),
                            userUUID, now, userUUID, now
                    )
            ).toList();
        ...
        if (!proposedCompetencyIdToRemove.isEmpty()) {
        ...
        if (!proposedCompetencyIdToCheck.isEmpty()) {
            Set<RoleCompetencyProposalItemId> roleCompetencyProposalToCheck = proposedCompetencyIdToCheck.stream().map(id ->
                    new RoleCompetencyProposalItemId(updatedRecord.getId().getProposalId(),
                            updatedRecord.getId().getRoleId(), updatedRecord.getId().getStaffId(), id)
            ).collect(Collectors.toSet());
            Map<RoleCompetencyProposalItemId, RoleCompetencyProposalItemDto> roleCompetencyProposalItemToUpdate = roleCompetencyProposalToCheck.stream().filter(
                    id -> !Objects.equals(
                            ...
                            selectedProposedCompetencyMap.get(id.getCompetencyProposalId()))
            ).collect(Collectors.toMap(Function.identity(), id -> {
        ...
        List<RoleCompetencyItemDto> roleCompetencyItemAfter = roleCompetencyItemService.findAllByProposalStaffId(
                req.getId().getProposalId(), req.getId().getStaffId());
        ...
        Set<String> duplicates = allCompetencyNameAfter.stream()
                .filter(name -> !seenAfter.add(name))
                .collect(Collectors.toSet());
        ...
        Map<String, Integer> roleCompetencyItemAfterMap = roleCompetencyItemAfter.stream().collect(Collectors.toMap(
                dto -> dto.getCompetency().getName(), RoleCompetencyItemDto::getWeightage));
        Map<String, Integer> roleCompetencyProposalItemAfterMap = roleCompetencyProposalItemAfter.stream().collect(Collectors.toMap(
                dto -> dto.getCompetencyProposal().getName(), RoleCompetencyProposalItemDto::getWeightage));
    ...
    @Transactional
    public void rejectCompetencyAssignmentProposal(RoleCompetencyProposalId selectId, UUID userUUID) throws Exception {
        if (selectId == null) {
        ...
        List<ProposalParticipantDto> allParticipantBefore = proposalParticipantService.getAllByProposalId(selectId.getProposalId());
        ProposalParticipantDto updatedBy = allParticipantBefore.stream()
                .filter(dto -> Objects.equals(dto.getId().getStaffId(), userUUID))
                .findAny().orElse(null);
        ...
        Map<String, Integer> roleCompetencyItemDeletedMap = roleCompetencyItemService
                ...
                .stream().collect(Collectors.toMap(dto -> dto.getCompetency().getName(), RoleCompetencyItemDto::getWeightage));

        Map<String, Integer> roleCompetencyProposalItemDeletedMap = roleCompetencyProposalItemService
                .findAndDeleteAllByProposalStaffId(selectId.getProposalId(), selectId.getStaffId())
                .stream().collect(Collectors.toMap(dto -> dto.getCompetencyProposal().getName(), RoleCompetencyProposalItemDto::getWeightage));

        List<JobScopeDto> deletedJobScopes = roleJobScopeProposalService
                .findAndDeleteAllByProposalStaffId(selectId.getProposalId(), selectId.getStaffId())
                .stream().map(RoleJobScopeProposalDto::getJobScope).toList();
        ...
        Set<ProposalParticipantDto> reviewers = allParticipantBefore.stream()
                .filter(dto -> Objects.equals(dto.getProposalRole(), ProposalRole.REVIEWER))
                .collect(Collectors.toSet());
        if (updatedBy != null) {
        ...
        if (existingRoleCompetencyProposal.isEmpty()) {
            Set<Long> deletedParticipantRoleIds = proposalParticipantService.findAndDeleteByProposalId(selectId.getProposalId())
    ...
    @Transactional
    public void bulkRejectCompetencyProposal(Set<RoleCompetencyProposalId> selectedIds, UUID userUUID) throws Exception {
        if (selectedIds == null) {
            String errorMessage = messageSource.getMessage(INVALID_REQUEST_BODY_ERR_MSG_CODE, new String[]{COMPETENCY_PROPOSAL_REJECT_OPERATION}, Locale.getDefault());
            throw new BadRequestException(errorMessage);
        }
        ...
        for (RoleCompetencyProposalDto deletedRecord : deletedRecords) {
            List<ProposalParticipantDto> recipients = allParticipants.stream().filter(dto ->
                    (Objects.equals(dto.getId().getProposalId(), deletedRecord.getId().getProposalId()) &&
                            Objects.equals(dto.getId().getStaffId(), deletedRecord.getId().getStaffId()))
                            || (Objects.equals(dto.getId().getProposalId(), deletedRecord.getId().getProposalId()) &&
                            Objects.equals(dto.getProposalRole(), ProposalRole.REVIEWER)) &&
                            (updatedBy == null || !Objects.equals(dto.getId().getStaffId(), updatedBy.getId().getStaffId()))
            ).toList();
            ...
            Map<String, Integer> deletedRoleCompetencyMap = deletedRoleCompetency.stream().filter(comp ->
                    ...
                    .collect(Collectors.toMap(comp -> comp.getCompetency().getName(), RoleCompetencyItemDto::getWeightage));

            Map<String, Integer> deletedRoleCompetencyProposalMap = deletedRoleCompetencyProposal.stream().filter(comp ->
                            Objects.equals(comp.getId().getProposalId(), deletedRecord.getId().getProposalId())
                                    && Objects.equals(comp.getId().getStaffId(), deletedRecord.getId().getStaffId()))
                    .collect(Collectors.toMap(comp -> comp.getCompetencyProposal().getName(), RoleCompetencyProposalItemDto::getWeightage));
        ...
        Set<ProposalParticipantId> participantsToBeDeleted = selectedIds.stream()
                .map(id -> new ProposalParticipantId(id.getProposalId(), id.getStaffId()))
                .collect(Collectors.toSet());
        Set<Long> deletedSelectedRoleIds = proposalParticipantService.findAndDeleteByIdIn(participantsToBeDeleted)
                .stream().map(dto -> dto.getStaff().getRole().getId()).collect(Collectors.toSet());
        ...
        for (RoleCompetencyProposalDto record : deletedRecords) {
            ...
            if (existingRoleCompetencyProposal.isEmpty()) {
                Set<Long> deletedParticipantRoleIds = proposalParticipantService.findAndDeleteByProposalId(record.getId().getProposalId())
                        .stream().map(dto -> dto.getStaff().getRole().getId()).collect(Collectors.toSet());
                deletedRoleIds.addAll(deletedParticipantRoleIds);
    ...
    @Transactional
    public void approveCompetencyAssignmentProposal(RoleCompetencyProposalId selectedId, UUID userUUID) throws Exception {
        if (selectedId == null) {
        ...
        List<RoleCompetencyItemDto> deletedRoleCompetency = roleCompetencyItemService.findAndDeleteAllByProposalId(selectedId.getProposalId());
        ...
        List<RoleCompetencyProposalItemDto> deletedRoleCompetencyProposal = roleCompetencyProposalItemService.findAndDeleteAllByProposalId(selectedId.getProposalId());
        ...
        Map<String, CompetencyDto> approvedCompetencyDtoMap = this.approveCompetencyFromProposal(
                approvedRoleCompetencyProposal, now, updatedBy, userUUID)
        ...
        List<RoleCompetencyDto> proposedRoleCompetencyDto = approvedRoleCompetencyProposal.stream().map(dto ->
                new RoleCompetencyDto(
                        ...
                        dto.getWeightage(), dto.getId().getStaffId(), now, userUUID, now)
        ).toList();
        ...
        List<RoleCompetencyDto> permanentRoleCompetencyDto = approvedRoleCompetency.stream().map(dto ->
                new RoleCompetencyDto(
                        ...
                        updatedRoleDto, dto.getCompetency(), dto.getWeightage(), dto.getId().getStaffId(), now, userUUID, now)
        ).toList();
        ...
        Set<Long> deletedProposedJobScopeIds = deletedRoleJobScope.stream().map(dto -> dto.getId().getJobScopeId()).collect(Collectors.toSet());

        List<RoleJobScopeDto> deletedExistingJobScope = roleJobScopeService.findAndDeleteAllByRoleId(selectedRole.getId());
        Set<Long> deletedPermanentJobScope = deletedExistingJobScope.stream().map(dto -> dto.getId().getJobScopeId()).collect(Collectors.toSet());
        ...
        List<RoleJobScopeDto> roleJobScopeToAdded = approvedRoleJobScope.stream().map(dto ->
                new RoleJobScopeDto(
                        ...
                        updatedRoleDto, dto.getJobScope(), dto.getId().getStaffId(), now, userUUID, now)
        ).toList();
        ...
        List<RoleCompetencyProposalDto> deletedRecords = roleCompetencyProposalService.findAndDeleteByProposalId(selectedId.getProposalId());
        ...
        for (ProposalParticipantDto recipient : recipients) {
            Map<String, Integer> approvedPermanentCompetencyMap = roleCompetencyCreated.stream()
                    .collect(Collectors.toMap(dto -> dto.getCompetency().getName(), RoleCompetencyDto::getWeightage));
        ...
        for (RoleCompetencyProposalDto rejected : rejectedRecords) {
            List<String> rejectedJobScopes = deletedRoleJobScope.stream().filter(dto ->
                    ...
                    .map(dto -> dto.getJobScope().getJobScope()).toList();

            Map<String, Integer> deletedPermanentCompetencyMap = deletedRoleCompetency.stream().filter(dto ->
                            Objects.equals(dto.getId().getProposalId(), rejected.getId().getProposalId())
                                    && Objects.equals(dto.getId().getStaffId(), rejected.getId().getStaffId()))
                    .collect(Collectors.toMap(dto -> dto.getCompetency().getName(), RoleCompetencyItemDto::getWeightage));

            Map<String, Integer> deletedProposedCompetencyMap = deletedRoleCompetencyProposal.stream().filter(dto ->
                            Objects.equals(dto.getId().getProposalId(), rejected.getId().getProposalId())
                                    && Objects.equals(dto.getId().getStaffId(), rejected.getId().getStaffId()))
                    .collect(Collectors.toMap(dto -> dto.getCompetencyProposal().getName(), RoleCompetencyProposalItemDto::getWeightage));
        ...
        Set<Long> removedStaffRoleIds = deletedParticipants.stream()
                .map(dto -> dto.getStaff().getRole().getId()).collect(Collectors.toSet());
    ...
    @Transactional
    public void bulkApproveCompetencyProposal(Set<RoleCompetencyProposalId> selectedIds, UUID userUUID) throws Exception {
        if (selectedIds == null) {
            String errorMessage = messageSource.getMessage(INVALID_REQUEST_BODY_ERR_MSG_CODE, new String[]{COMPETENCY_PROPOSAL_APPROVE_OPERATION}, Locale.getDefault());
            throw new BadRequestException(errorMessage);
        }
        ...
        Set<Long> proposalIdsDuplicates = selectedProposalIdList.stream()
                .filter(id -> !proposalIdSeen.add(id)).collect(Collectors.toSet());
        if (!proposalIdsDuplicates.isEmpty()) {
        ...
        Set<Long> roleIdDuplicates = selectedRoleIdList.stream()
                .filter(id -> !roleIdSeen.add(id)).collect(Collectors.toSet());
        if (!roleIdDuplicates.isEmpty()) {
        ...
        List<RoleCompetencyItemDto> approvedPermanentCompetency = deletedPermanentCompetency.stream().filter(dto ->
                selectedIds.contains(new RoleCompetencyProposalId(dto.getId().getProposalId(), dto.getId().getRoleId(), dto.getId().getStaffId()))
        ).toList();
        ...
        Set<Long> rejectedJobScopeIds = rejectedRoleJobScopeProposal.stream().map(dto -> dto.getJobScope().getId()).collect(Collectors.toSet());
        ...
        List<RoleCompetencyProposalDto> approvedRecords = selectedRecords.stream().filter(dto -> selectedIds.contains(dto.getId())).toList();
        List<RoleCompetencyProposalDto> rejectedRecords = new ArrayList<>(selectedRecords);
        rejectedRecords.removeAll(approvedRecords);
        Map<Long, RoleCompetencyProposalDto> selectedRecordsMapByRoleId = approvedRecords.stream()
                .collect(Collectors.toMap(dto -> dto.getId().getRoleId(), Function.identity()));
        ...
        Map<Long, RoleDto> updatedRoleMap = updatedRoleDtos.stream().collect(Collectors.toMap(RoleDto::getId, Function.identity()));

        List<RoleJobScopeDto> deletedRoleJobScope = roleJobScopeService.findAndDeleteAllByRoleIdIn(selectedRoleIds);
        Set<Long> deletedRoleJobScopeIds = deletedRoleJobScope.stream().map(dto -> dto.getJobScope().getId()).collect(Collectors.toSet());
        ...
        List<RoleJobScopeDto> roleJobScopeToBeCreated = approvedRoleJobScopeProposal.stream().map(dto ->
                new RoleJobScopeDto(
                        ...
                        updatedRoleMap.get(dto.getId().getRoleId()), dto.getJobScope(),
                        dto.getId().getStaffId(), now, userUUID, now)
        ).toList();
        ...
        List<RoleCompetencyDto> permanentRoleCompetencyToBeAdded = approvedPermanentCompetency.stream().map(dto ->
                new RoleCompetencyDto(
                        new RoleCompetencyId(dto.getId().getRoleId(), dto.getId().getCompetencyId()),
                        updatedRoleMap.get(dto.getId().getRoleId()), dto.getCompetency(),
                        dto.getWeightage(), dto.getId().getStaffId(), now, userUUID, now)
        ).toList();
        ...
        for (RoleCompetencyProposalDto approved : approvedRecords) {
            List<String> recipientEmails = allParticipants.stream().filter(dto ->
                            (Objects.equals(dto.getId().getProposalId(), approved.getId().getProposalId()))
                                    ...
                                    || (Objects.equals(dto.getProposalRole(), ProposalRole.REVIEWER)
                                    && (!Objects.equals(dto.getId().getStaffId(), updatedBy.getId().getStaffId())))))
                    .map(dto -> dto.getStaff().getEmail()).toList();
            ...
            for (String recipientEmail : recipientEmails) {
                emailService.sendCompetencyAssignmentProposalApprovedEmail(
                        ...
                        approvedJobScopes, approvedCompetencyMap, updatedBy.getStaff().getEmail(),
                        Locale.getDefault()
                );
            }
        }
        ...
        for (RoleCompetencyProposalDto rejected : rejectedRecords) {
            ...
            for (String recipientEmail : recipientEmails) {
                emailService.sendCompetencyAssignmentProposalApprovedEmail(
                        ...
                        rejectedJobScopes, rejectedCompetencyMap, updatedBy.getStaff().getEmail(),
                        Locale.getDefault()
                );
            }
        }
    ...
    private void removeGrantedAccessIfNotLongerUsed(Set<Long> roleId) {
        ...
        if (!toRemove.isEmpty()) {
            Set<RoleAuthorityId> roleAuthorityIdToRemove = toRemove.stream()
                    .map(id -> new RoleAuthorityId(id, authorityDto.getId())).collect(Collectors.toSet());
    ...
    private List<CompetencyDto> approveCompetencyFromProposal(List<RoleCompetencyProposalItemDto> approvedRoleCompetencyProposal,
        ...
        List<CompetencyCompTagProposalDto> approvedCompetencyCompTagProposal = deletedCompetencyCompTagProposal.stream().filter(
        ...
        List<CompetencyProposalDto> approvedCompetencyProposal = deletedCompetencyProposal.stream()
                .filter(dto -> approvedCompetencyProposalIds.contains(dto.getId())).toList();
        ...
        for (CompetencyProposalDto approved : approvedCompetencyProposal) {
            List<String> approvedCompTag = deletedCompetencyCompTagProposal.stream()
                    .filter(compTag -> Objects.equals(compTag.getId().getProposalId(), approved.getId().getProposalId())
            ...
            List<String> recipientEmail = allParticipants.stream().filter(participant ->
                    (Objects.equals(participant.getId().getProposalId(), approved.getId().getProposalId())
                            && Objects.equals(participant.getId().getStaffId(), approved.getId().getStaffId())
                            || (Objects.equals(participant.getProposalRole(), ProposalRole.REVIEWER))
                            && (updatedBy == null || !Objects.equals(updatedBy.getId(), participant.getId())))
            ).map(participant -> participant.getStaff().getEmail()).toList();

            for (String recipient : recipientEmail) {
                emailService.sendCompetencyProposalApprovedEmail(
                        recipient, approved.getName(), approved.getDescription(), approvedCompTag,
                        updatedBy == null ? "Email not Found" : updatedBy.getStaff().getEmail(),
                        Locale.getDefault()
                );
            }
        ...
        for (CompetencyProposalDto rejected : rejectedCompetencyProposal) {
            List<String> rejectedCompTag = deletedCompetencyCompTagProposal.stream()
                    .filter(compTag -> Objects.equals(compTag.getId().getProposalId(), rejected.getId().getProposalId())
                            && Objects.equals(compTag.getId().getStaffId(), rejected.getId().getStaffId()))
                    .map(compTag -> compTag.getCompTag().getTag()).toList();

            emailService.sendCompetencyProposalRejectedEmail(
                    rejected.getStaff().getEmail(), rejected.getName(), rejected.getDescription(), rejectedCompTag,
                    updatedBy == null ? "Email not Found" : updatedBy.getStaff().getEmail(),
                    Locale.getDefault()
            );
        }
        ...
        List<CompetencyDto> competencyToBeAdded = approvedCompetencyProposal.stream()
                .map(dto -> new CompetencyDto(dto.getName(), dto.getDescription(), false,
                        dto.getId().getStaffId(), now, userUUID, now))
                .collect(Collectors.toList());
        ...
        List<CompetencyCompTagDto> competencyCompTagToBeAdded = competencyDtoCreated.stream().flatMap(dto -> {
            ...
            if (competencyProposalIdBefore != null) {
                List<CompTagDto> assignedCompTag = approvedCompetencyCompTagProposal.stream().filter(
                        ...
                        .map(CompetencyCompTagProposalDto::getCompTag).toList();

                List<CompetencyCompTagDto> toAdd = assignedCompTag.stream().map(compTag ->
                        new CompetencyCompTagDto(
                                new CompetencyCompTagId(dto.getId(), compTag.getId()),
                                dto, compTag, dto.getCreatedBy(), now, userUUID, now)
                ).toList();
                return toAdd.stream();
            } else {
                return null;
