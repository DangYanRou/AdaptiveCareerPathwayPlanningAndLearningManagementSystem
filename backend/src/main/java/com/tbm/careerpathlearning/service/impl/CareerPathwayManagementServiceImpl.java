...
@Service
public class CareerPathwayManagementServiceImpl implements CareerPathwayManagementService {
    ...
    @Override
    @Transactional
    public List<CareerPathwayOverviewDto> getOverview() {
        ...
        return existingRecords.stream().map(dto -> {
    ...
    @Override
    @Transactional
    public void create(CreateCareerPathwayRequestDto req, UUID userId) {
    ...
    @Override
    @Transactional
    public void update(CreateCareerPathwayRequestDto req, UUID userId) {
        if (req == null || req.getRootRoleId() == null || req.getCareerPathwayId() == null || req.getOrgChartId() == null
                || validationService.isNullOrBlank(req.getCareerPathwayName()) || req.getRootRoleId() == null) {
        ...
        selectedRecord.setUpdatedBy(userId);
        ...
        if (!trackToRemove.isEmpty()) {
            ...
            removeTrackIfNotLongerUsed(trackToRemove, userId, now);
        }
        ...
        Map<CareerPathwayRoleId, CareerPathwayRoleDto> selectedNodeMap = selectedNodes.stream()
                .collect(Collectors.toMap(CareerPathwayRoleDto::getId, Function.identity()));
    ...
    @Override
    @Transactional
    public void delete(Long selectedCareerPathwayId, UUID userId) {
        ...
        careerPathwayService.delete(selectedCareerPathwayId, userId, now);

        Set<Long> deletedTrackIds = careerPathwayTrackService.findAndDeleteByCareerPathwayId(selectedCareerPathwayId)
                .stream().map(dto -> dto.getId().getTrackId()).collect(Collectors.toSet());
        removeTrackIfNotLongerUsed(deletedTrackIds, userId, now);
    ...
    @Override
    @Transactional
    public void bulkDelete(Set<Long> selectedCareerPathwayIds, UUID userId) {
        ...
        careerPathwayService.deleteAll(selectedCareerPathwayIds, userId, now);
    ...
    public void importData(MultipartFile file, UUID userId) throws IOException {
        ...
        for (int i = 1; i <= sheet.getLastRowNum(); i++) {
            ...
            if (toBeDeleted != null && toBeDeleted.equalsIgnoreCase(YES) &&
                    !validationService.isNullOrBlank(newCareerPathwayName)) {
        ...
        if (!careerPathwayTagIdsToBeDeleted.isEmpty()) {
            ...
            trackService.deleteAllByIdIn(toRemove, userId, now);
        }
    ...
    @Override
    @Transactional
    public CareerPathwayOverviewDto getMyCareerPathway(UUID staffId) {
    ...
    private CareerPathwayNodeDto buildVisualisedGraph(
        ...
        if (id.equals(currentRoleId)) {
            data.setHasVisited(0);
        } else if (visitedRoles.contains(id)) {
            data.setHasVisited(1);
        } else {
            data.setHasVisited(-1);
        }
    ...
    private void removeTrackIfNotLongerUsed(Set<Long> toRemove, UUID userId, OffsetDateTime now) {
        ...
        trackService.deleteAllByIdIn(toDelete, userId, now);
    }
